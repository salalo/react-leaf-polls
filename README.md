...
| *isVotedId  |                    Number                     |    ✖     |          false          | When an id is set, poll will indicate with the main theme color the result which the user has already voted on. To be used in conjunction with isVoted. ***Only enabled for multiple**.     |
